...
def process_song_file(cur, filepath):
    ...
    cur.executemany(song_table_insert, [song for song in song_data])
    
    # insert artist record
    artist_data = df[['artist_id', 'artist_name', 'artist_location', 'artist_latitude', 'artist_longitude']].values.tolist()
    
    cur.executemany(artist_table_insert, [artist for artist in artist_data])
...
def process_log_file(cur, filepath):
    ...
    user_df.replace('^\s*$', np.nan, regex=True, inplace=True)

    # Drop rows where userId is NaN
    user_df.dropna(axis=0, subset=['userId'],inplace=True)

    # insert user records
    for i, row in user_df.iterrows():
        cur.execute(user_table_insert, row)

    # Convert ts to datetime to match time table
